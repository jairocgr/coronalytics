...
class IngestSragFileJob < ApplicationJob
  ...
  def perform(srag)
    ...
    begin
      ...
      srag.update! status: 'INGESTED'
    ensure
      srag.update status: 'ERROR' unless srag.status == 'INGESTED'
    end
  end
...
private
  ...
  def download(srag)
    ...
    srag.update! status: 'DOWNLOADING'
    src = URI.open(srag.url)
    IO.copy_stream(src, srag.local_path)

    srag.update! status: 'DOWNLOADED', downloaded: true
